...
int cConnectionHandlerInbound::ProcessHTTP(cConnectionHandler::ProtocolType &pt)
{
	...
	if (pMessage)
	{
		...
		// For now we don't want to do this for HTTP messages
		//
		//cPeerRoutingMap *pRouteMap = m_pConnectionQueue->GetPeerRoutingMap();
		//pRouteMap->AddRecord(pMessage->GetFrom(), pMessage->GetSourceIP(), pMessage->GetSourcePort(), m_pConnectionRecord->Socketfd);
		...
		m_pConnectionQueue->GetAccessLog()->AddEntry(pMessage->GetProtocolType(), pMessage->GetMsgPriority(),
										pMessage->GetCommand(), "Website access",
										pMessage->GetSourceIP());
...
int cConnectionHandlerInbound::ProcessMCP(cConnectionHandler::ProtocolType &pt)
{
	...
	if (pMessage)
	{
		...
		if (rc == HRC_MSG_OK)
		{
			...
			m_pConnectionQueue->GetAccessLog()->AddEntry(pMessage->GetProtocolType(), pMessage->GetMsgPriority(),
											pMessage->GetCommand(), "MC Message",
											pMessage->GetSourceIP());
			rc = HRC_CONNECTION_HANDLER_ENDTASK;
		}
		else
