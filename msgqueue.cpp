...
/*
 * SQLite documentations states: Windows systems should call this routine
 * to free memory that is returned in the in the errmsg parameter of
 * sqlite3_open() when SQLite is a DLL.  For some reason, it does not
 * work to call free() directly.  See SQLite's main.c
 */
#ifdef  _PLATFORM_WIN32
	#define SQLITE_ERROR_FREE(mem) sqlite3_free(mem)
#else
	#define SQLITE_ERROR_FREE(mem) free(mem)
 ...
 * processing
 */
cMsgQueue::cMsgQueue(cCommandHandlerFactory *pFactory, cMsgRouterRulesEngine *pRouterRulesEngine, cService *pServiceEngine)
: m_pCommandHandlerFactory(pFactory)
, m_pRouterRulesEngine(pRouterRulesEngine)
, m_pServiceEngine(pServiceEngine)
, m_pSysMetrics(0)
, m_bUseDB(false)
{
...
int cMsgQueue::Run()
{
    ...
    while (ThreadRunning())
    {
		...
		// check to see if it's time to flush messages to disk
		if (m_timer.IsReady())
		{
			if (m_bUseDB)
				FlushMessagesToDB();
...
int cMsgQueue::UseDatabase(char *pName, int iFlushInterval)
{
	int rc;
	m_sDatabaseName = pName;
	m_FlushToDiskInterval = iFlushInterval;

	sqlite3_open(m_sDatabaseName.c_str(), &m_dbFile);
	if (sqlite3_errcode(m_dbFile) != SQLITE_OK)
	{
		//LOG2("%s", sqlite3_errmsg(m_dbFile));
		rc = HRC_MSG_QUEUE_DB_FAILED;
	}
	else if ((rc = CheckDB()) != HRC_MSG_QUEUE_OK)
	{
		rc = HRC_MSG_QUEUE_DB_FAILED;
	}
	else
	{
		m_bUseDB = true;
		m_timer.SetInterval(m_FlushToDiskInterval);
		m_timer.Start();
		rc = HRC_MSG_QUEUE_OK;
	}
...
int cMsgQueue::CheckDB()
{
	...
	char buffer[100];
	FILE *fp;
	fp = fopen(m_sDatabaseName.c_str(), "r+b");
	if (fp)
	{
		iRead = fread(buffer, 1, 100, fp);
		if (iRead == 0)
			bCreate = true;
		fclose(fp);
	}
...
int cMsgQueue::FormatDB(sqlite3 *db)
{
	char *tblMsg =
	"CREATE TABLE tblMsg"
	"("
	"	fldMsgID		VARCHAR(37),"		// Message GUID
	"   fldMsgConnectionType INT,"			// IBUDP, OBUDP, INTCP, OBTCP
	"   fldMsgFormat	INT,"				// REQ/RES
	"	fldMsgResponseCode	INT,"			// Message response code if message is not a request
	"	fldMsgState		INT,"				// Pending, Undelivered, Delivered
	"	fldCommand		VARCHAR(255),"		// Message Command type: PRESENCE, CHAT etc...
	"	fldTo			VARCHAR(255),"		// Destination server
	"	fldFrom			VARCHAR(255),"		// From server
	"	fldCRC32		INT,"				// CRC-32 checksum value for content payload
	"	fldContentType	VARCHAR(40),"		// MIME content type: text/xml, text/base64, etc...
	"	fldContentLength INT,"				// Actual size of payload content
	"	fldPayload		BLOB"				// payload content
	");";

	int rc;
	char *szErrorMsg = 0;

	if ((rc = sqlite3_exec(db, tblMsg, NULL, 0, &szErrorMsg)) != SQLITE_OK)
	{
		//LOG2("Database error=%d (%s)", rc, szErrorMsg);
		SQLITE_ERROR_FREE(szErrorMsg);
		return HRC_MSG_QUEUE_DB_TABLE_CREATE_FAILED;
	}

	// create database indexes
	//
	rc = sqlite3_exec(db, "CREATE INDEX idxMsg ON tblMsg(fldMsgID ASC);", NULL, 0, &szErrorMsg);
	if (rc != SQLITE_OK)
	{
		//LOG2("Database error=%d (%s)", rc, szErrorMsg);
		SQLITE_ERROR_FREE(szErrorMsg);
...
int cMsgQueue::FlushMessagesToDB()
{
	int rc;
	sqlite3_stmt *pStmt;

	rc = sqlite3_prepare(m_dbFile, "INSERT INTO tblMsg VALUES(?,?,?,?,?,?,?,?,?,?,?,?)", -1, &pStmt, 0);
	if (rc != SQLITE_OK && pStmt == NULL)
		return HRC_MSG_QUEUE_DB_FAILED;
	...
	{
		msgState = pMsg->GetMsgState();
		if (msgState == cIMsg::MSG_STATE_UNDELIVERED ||
			msgState == cIMsg::MSG_STATE_DELIVERED ||
			msgState == cIMsg::MSG_STATE_PROCESSED)
		{
			// bind fields
			//
			sqlite3_bind_text(pStmt, 1, pMsg->GetMsgID(), -1, SQLITE_STATIC); // fldMsgID
			sqlite3_bind_int(pStmt,  2, pMsg->GetConnectionType()); //  fldMsgConnectionType
			sqlite3_bind_int(pStmt,  3, pMsg->GetFormat()); // fldMsgFormat
			sqlite3_bind_int(pStmt,  4, pMsg->GetResponseCode()); // fldMsgResponseCode
			sqlite3_bind_int(pStmt,  5, pMsg->GetMsgState()); // fldMsgState
			sqlite3_bind_text(pStmt, 6, pMsg->GetCommand(), -1, SQLITE_STATIC); // fldCommand
			sqlite3_bind_text(pStmt, 7, pMsg->GetTo(), -1, SQLITE_STATIC); // fldTo
			sqlite3_bind_text(pStmt, 8, pMsg->GetFrom(), -1, SQLITE_STATIC); // fldFrom
			sqlite3_bind_int(pStmt,  9, atoi(pMsg->GetCRC32())); // fldCRC32
			sqlite3_bind_text(pStmt,10, pMsg->GetContentType(), -1, SQLITE_STATIC); // fldContentType
			sqlite3_bind_int(pStmt, 11, pMsg->GetContentLength()); // fldContentLength
			sqlite3_bind_blob(pStmt,12, pMsg->GetContentPayload(), pMsg->GetContentLength(), SQLITE_STATIC);  //fldPayload		BLOB"				// payload content
			...
			if (rc == SQLITE_DONE)
			{
				// if database flush was successful mark message
				// as not in use so that it can be recycled
				MARK_TRAVEL(pMsg);
				pMsg->SetMsgState(cMsg::MSG_STATE_MARKFORREUSE);
			}
			else
