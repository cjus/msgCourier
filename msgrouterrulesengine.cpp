...
bool cMsgRouterRulesEngine::TestMatch(cMsg *pMsg, cMsgRouterRule *pRule)
{
	try
	{
		if (pRule->m_Location == cMsgRouterRule::MSG_RTL_STATUS)
		{
			...
			cmatch what;
			const regex expression(pRule->m_MatchPattern.c_str()); 
			if (regex_match(pLine, what, expression, match_default) == true) 
				return true;
		}
		else if (pRule->m_Location == cMsgRouterRule::MSG_RTL_HEADER)
		{
			...
			regex expression(pRule->m_MatchPattern.c_str()); 
			cmatch what;	
			if (regex_match(header.c_str(), what, expression)) 
				return true;
		}
		else if (pRule->m_Location == cMsgRouterRule::MSG_RTL_PAYLOAD)
		{
