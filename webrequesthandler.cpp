...
int cWebRequestHandler::Setup(cVirtualDirectoryMap *pVirtualDirectoryMap, cScriptEngine *pScriptEngine, cIMsgQueue *pMsgQueue, cIMsg *pMessage)
{
	m_bConnected = true;
	...
	m_pIMsg = pMessage;
	cUniqueIDProvider::GetInstance()->GetID(m_sInterpreterNamespace);
	return HRC_WEBREQUEST_HANDLER_HANDLER_OK;
}
...
int cWebRequestHandler::Process(cIMsg *pMessage)
{
	try
	{
		...
		// Process embedded script, if any
		if (m_CGIScript.empty() == false)
			ProcessEmbeddedScript();
...
//	bool bRet = false;
	...
	try
	{
		...
		tokenizer tokens(decURI, sep);
		for (tokenizer::iterator tok_iter = tokens.begin();
			tok_iter != tokens.end(); ++tok_iter)
		{
			if ((*tok_iter).find('=') != string::npos)
				m_CGIParams = *tok_iter;
			else
				m_URI.push_back(*tok_iter);
		}
		int iURIElements = m_URI.size();
		if (iURIElements==0)
		{
			string defaultpage = "index.html";
			m_URI.push_back(defaultpage);
			iURIElements=1;
		}
		...
		if (m_bMessageTypeGet == false)
		{
			...
			if (length > 0)
			{
				...
				{
					string sPattern = "tclscript=";
					string::size_type pos = decURI.find(sPattern);
					if (pos != string::npos)
					{
						m_CGIScript = decURI;
						m_CGIScript.erase(m_CGIScript.begin(), m_CGIScript.begin() + pos + sPattern.length());
						if (m_CGIScript.size() == 0)
							m_CGIScript = "";
						cBuffer buf;
						buf.ReplaceWith((char*)m_CGIScript.c_str());
						buf.ReplaceAll("<PRE>","\n");	buf.ReplaceAll("<pre>","\n");
						buf.ReplaceAll("</PRE>","\n");	buf.ReplaceAll("</pre>","\n");
						buf.ReplaceAll("<BR>","\n");	buf.ReplaceAll("<br>","\n");
						buf.ReplaceAll("<P>","");		buf.ReplaceAll("<p>","");
						buf.ReplaceAll("</P>","\n");	buf.ReplaceAll("</p>","\n");
						buf.ReplaceAll("&nbsp;"," ");
						buf.ReplaceAll("&lt;","<");
						buf.ReplaceAll("&gt;",">");
...
void cWebRequestHandler::ProcessServerSideScripts()
{
	...
	try
	{
		if (m_Response.Find("<?mc")!=-1)
		{
			// Process CGI params
			m_CGI.erase(m_CGI.begin(), m_CGI.end());
			string setVar, setOut;
			string sVar, sVal;
			string sTemp;
			typedef tokenizer<char_separator<char> > tokenizer;
			char_separator<char> sep("?&");
			tokenizer tokens(m_CGIParams, sep);
			for (tokenizer::iterator tok_iter = tokens.begin();
				tok_iter != tokens.end(); ++tok_iter)
			{
				sTemp = *tok_iter;
				string::size_type i = sTemp.find("=");
				if (i != string::npos)
				{
					sVar.assign(sTemp, 0, i);
					sVal.assign(sTemp, i+1, sTemp.length()-1-i);
					setVar = "set " + sVar;
					setVar += " {";
					setVar += sVal;
					setVar += "}";
					m_pScriptEngine->Execute(m_sInterpreterNamespace, setVar, setOut, false);
					m_CGI.push_back(sVar);
				}
			}

			// Process script
			int scriptend = 0;
			string::size_type endOfFirstLine;
			string::size_type beginDirective;
			int scriptheaderpattern = strlen("<?mc");
			int scripttailpattern = strlen("?>");
			const char *pSilentDirective = " -silent";
			string firstLine;
			string script, output;
			bool bSilent = false;
			while ((index = m_Response.Find("<?mc"))!= -1)
			{
				// get end of script block
				scriptend = m_Response.Find("?>",index);
				if (scriptend == -1)
					break;

				script.assign(m_Response.cstr() + index + scriptheaderpattern,
								scriptend - index - scriptheaderpattern);

				// check for script directives
				endOfFirstLine = script.find("\n");
				if (endOfFirstLine != string::npos)
				{
					firstLine.assign(script.c_str(), endOfFirstLine);
					beginDirective = firstLine.find(pSilentDirective);
					if (beginDirective != string::npos)
					{
						bSilent = true;
						beginDirective = script.find(pSilentDirective);
						script.erase(beginDirective, strlen(pSilentDirective));
					}
				}

				// process script
				m_pScriptEngine->Execute(m_sInterpreterNamespace, script, output, bSilent);

				// remove server side script
				m_Response.Remove(index, (scriptend + scripttailpattern) - index);

				// insert interpreter response if any
				if (!output.empty())
					m_Response.Insert(index, output.c_str(), output.length());

				// toggle silent flag back to false
				bSilent = false;
			}

			m_pScriptEngine->DeleteNamespace(m_sInterpreterNamespace, output);
		}
...
void cWebRequestHandler::ProcessEmbeddedScript()
{
	try
	{
		if (m_CGIScript.empty())
			return;

		string output;
		string store;
		m_pScriptEngine->Execute(m_sInterpreterNamespace, m_CGIScript, output, false);

		cURL urlEnc;
		if (!output.empty())
			urlEnc.Encode(output, output);

		store = "set tclscriptout {";
		if (!output.empty())
			store += output.c_str();
		store += "}";
		m_pScriptEngine->Execute(m_sInterpreterNamespace, store, output, false);
	}
	catch (exception const &e)
	{
		LOGALL(e.what());
