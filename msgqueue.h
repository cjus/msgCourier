...
{
public:
    cMsgQueue(cCommandHandlerFactory *pFactory, cMsgRouterRulesEngine *pRouterRulesEngine, cService *pServiceEngine);
    ~cMsgQueue();
...
private:
	...
	int CheckDB();
	int FormatDB(sqlite3 *db);
	int FlushMessages();
	int FlushMessagesToDB();
