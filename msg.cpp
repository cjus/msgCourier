...
/*
	if ((MsgReplyActionType == cMsg::MSG_REPLY_ACTION_NONE || MsgReplyActionType == cMsg::MSG_REPLY_ACTION_NOWAIT) &&
		MsgNotifyActionType == cMsg::MSG_NOTIFY_ACTION_NO)
	{
		this->SetTCPSocketHandle(0);
	}
*/
		...
		/*
		int idx = MsgCommand.compare(0, 7, "MCP/");
		if (idx != -1)
			MsgFormat = cMsg::MSG_FORMAT_RES;
		*/
...
void cMsg::SetRemoteHandler(cServiceEntry *pService)
{
	cAutoThreadSync ThreadSync(&m_ThreadSync);
	MsgService = pService;
}

cServiceEntry *cMsg::GetRemoteHandler()
{
	return MsgService;
}
