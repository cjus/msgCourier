...
int cPorterStemming::Process(char *pszString) 
{
	m_InBuffer.ReplaceWith(pszString);
	m_OutBuffer.Reset();

	char *seps = " |<>@!#$%^'`&*()~:;\".,_-+=\t?";
	char *p, *pWord;
	int len;
	p = strtok(m_InBuffer.cstr(), seps);
	while (p != NULL)
	{
		m_Word.ReplaceWith(p);
		pWord = m_Word.cstr();
		len = stem(pWord, 0, strlen(pWord)-1);
		if (len == 0)
			break;
		
		*(pWord+len+1) = 0;
		m_Word.MakeString();
		m_OutBuffer.Append(m_Word.cstr());
		m_OutBuffer.Append(" ");
		p = strtok(NULL, seps);
	}
	...
	if (p!=NULL)
	{		
		p[m_OutBuffer.StringLen()-1] = 0;
		m_OutBuffer.MakeString();
		strcpy(pszString, m_OutBuffer.cstr());
	}
...
int cPorterStemming::cons(int i)
{  switch (b[i])
...
int cPorterStemming::vowelinstem()
...
int cPorterStemming::doublec(int j)
...
int cPorterStemming::ends(char * s)
{  int length = s[0];
...
void cPorterStemming::setto(char * s)
{  int length = s[0];
...
void cPorterStemming::r(char * s) { if (m() > 0) setto(s); }
...
void cPorterStemming::step1c() { if (ends("\01" "y") && vowelinstem()) b[k] = 'i'; }
...
void cPorterStemming::step2() { switch (b[k-1])
{
...
void cPorterStemming::step3() { switch (b[k])
{
...
int cPorterStemming::stem(char * p, int i, int j)
